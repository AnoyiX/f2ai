...
- **多媒体文件 (Video/Audio)**
  ...
  - 音频文件支持自动转换为文本（需开启 `enableA2T`），基于 Faster Whisper 模型。
  - 支持自定义抽帧间隔、图片缩放及语音识别语言。
  - 原始文件直接存储并返回访问地址。
...
docker run -d -p 8000:8000 -v ~/static:/app/static -e API_TOKEN=your_secret_token --name f2ai f2ai:latest
```
...
| 参数名          | 类型    | 必选 | 默认值 | 说明                                                   |
| :-------------- | :------ | :--- | :----- | :----------------------------------------------------- |
| `token`         | String  | 否   | -      | API 认证 Token (如果服务端设置了 `API_TOKEN` 环境变量，则此项必填) |
| `file`          | File    | 是   | -      | 需要上传的文件                                         |
| `imgW`          | Integer | 否   | 1024   | 图片最大宽度（像素），仅当原图尺寸超过该值时才会缩小。 |
| `imgH`          | Integer | 否   | 1024   | 图片最大高度（像素），仅当原图尺寸超过该值时才会缩小。 |
| `enbaleV2I`     | Boolean | 否   | True   | 是否开启视频转图片功能。                               |
| `videoFPS`      | Float   | 否   | 1.0    | 视频截帧间隔（秒），例如 1.0 表示每秒截取一帧。        |
| `enableA2T`     | Boolean | 否   | True   | 是否开启音频转文本功能。                               |
| `audioLanguage` | String  | 否   | -      | 语音识别语言（如 'zh', 'en'），不填，则默认自动检测。  |
